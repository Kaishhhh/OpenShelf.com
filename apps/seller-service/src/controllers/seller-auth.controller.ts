...
export async function register(req: Request, res: Response) {
  ...
  void sendOtpEmail({ to: email, code: otp, purpose: 'seller' });
...
export async function resendOtp(req: Request, res: Response) {
  ...
  const pending = await getPendingRegistration<PendingSellerRegistration>(
    NAMESPACE,
    email
  );
...
export async function refreshToken(req: Request, res: Response) {
  ...
  const check = await checkRefreshToken(NAMESPACE, decoded, async (id) =>
    Boolean(await prisma.seller.findUnique({ where: { id }, select: { id: true } }))
  );
  if (check === 'foreign') {
